...
export class CategoryFormComponent implements OnInit {
  ...
  ngOnInit() {
    this.form = this.fb.group({ name: ['', Validators.required], status: ['', Validators.required] });
    this.id = this.route.snapshot.params['id'] ? +this.route.snapshot.params['id'] : null;
    this.isEdit = !!this.id;
    if (this.isEdit) {
      this.loading = true;
      this.svc.getCategory(this.id!).subscribe({
  ...
  goBack() { this.router.navigate(['/product/category/list']); }

  save() {
    if (this.form.invalid) { this.form.markAllAsTouched(); return; }
    this.saving = true;
    const obs = this.isEdit ? this.svc.updateCategory(this.id!, { ...this.form.value, status: +(this.form.value.status ?? 0) }) : this.svc.saveCategory({ ...this.form.value, status: +(this.form.value.status ?? 0) });
