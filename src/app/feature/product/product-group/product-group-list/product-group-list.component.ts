...
import { ProductGroup } from '../../../../shared/models/product.model';
...
export class ProductGroupListComponent implements OnInit {
  displayedColumns = ['id', 'code', 'name', 'invoice_group', 'status', 'actions'];
  data: ProductGroup[] = [];
...
ngOnInit() { this.load(); }
  ...
  load() {
    this.loading = true;
    this.svc.getProductGroups(this.page, this.pageSize, this.search).subscribe({
  ...
  add() { this.router.navigate(['/product/productgroup/add']); }
  edit(row: ProductGroup) { this.router.navigate(['/product/productgroup/edit', row.id]); }
  delete(row: ProductGroup) {
    if (!confirm(`Delete group "${row.name}"?`)) return;
    this.svc.deleteProductGroup(row.id!).subscribe(() => this.load());
  }
}
